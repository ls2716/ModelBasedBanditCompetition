...
def simulate_game(bandit_1, bandit_2, results_folder, bandit_1_name, bandit_2_name, simulation_setup):
    ...
    logger.info('Bandit in prior xks')
    if hasattr(bandit_1.model, 'prior_xks'):
        logger.info(bandit_1.model.prior_xks)
    else:
        logger.info('None')
    logger.info('Bandit entering prior xks')
    # If model has attribute prior_xks
    if hasattr(bandit_2.model, 'prior_xks'):
        logger.info(bandit_2.model.prior_xks)
    else:
        logger.info('None')
    # Run simulations
    ...
    np.savetxt(os.path.join(results_folder, 'bandit_2_action_frequencies.csv'),
               bandit_2_action_frequencies, delimiter=',')
