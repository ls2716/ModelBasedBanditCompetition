...
def plot_smooth_reward_history(reward_history, bandit1_name, bandit2_name,
    ...
    # plt.ylim(np.min(reward_history)-0.1, np.max(reward_history)+0.1)
    plt.ylim(-0.12, 1.47)
    if title is not None:
        plt.title(title)
    plt.tight_layout()
    plt.savefig(os.path.join(foldername, filename + '_reward_history.png'))
    plt.savefig(os.path.join(foldername, filename + '_reward_history.pdf'))
    if show_plot:
        plt.show()
    plt.close()
...
def plot_action_history(action_set, action_history, foldername, filename, title=None, show_plot=True):
    ...
    plt.savefig(os.path.join(foldername, filename + '_action_frequencies.pdf'))
    if show_plot:
        plt.show()
    plt.close()
