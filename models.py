...
class StationaryLogisticModel(BaseModel):
    ...
    def __init__(self, candidate_margins, dimension=2, variance=10**3) -> None:
        ...
        self.mean_0 = np.zeros(shape=(self.dimension, 1))
    ...
    def set_prior(self, prior_mean, prior_cov):
        self.mean_0 = prior_mean
        self.cov_0 = prior_cov
        self.reset()

    def reset(self):
        """Reset model."""
        self.mean = self.mean_0
        self.cov = self.cov_0
        self.xks = None
        self.Ys = None
        self.weights = None
    ...
    def set_prior_observations(self, margins, observations):
        self.prior_xks = np.ones((self.dimension, len(margins)))
        for i in range(len(margins)):
            for dim in range(1, self.dimension):
                self.prior_xks[dim, i] = margins[i]**dim
        self.prior_Ys = observations
        self.prior_weights = np.ones_like(self.prior_Ys)
    ...
    def compute_mean_and_cov(self, m_threshold=50, cov_threshold=50):
        if np.max(np.abs(self.cov)) > cov_threshold or np.max(np.abs(self.mean)) > m_threshold:
            # self.mean = np.zerios_like(self.mean)
            self.mean = self.mean_0
        if self.xks is None:
            self.update_xks = self.prior_xks
            self.update_Ys = self.prior_Ys
            self.update_weights = self.prior_weights
        elif self.prior_xks is None:
            self.update_xks = self.xks
            self.update_Ys = self.Ys
            self.update_weights = self.weights
        else:
            self.update_xks = np.hstack((self.prior_xks, self.xks))
            self.update_Ys = np.append(self.prior_Ys, self.Ys)
            self.update_weights = np.append(self.prior_weights, self.weights)

        mutxks = self.mean.T @ self.update_xks
        self.S_t = np.zeros_like(self.cov_0)
        self.S_t_factors = sigmoid(mutxks)*(
            1-sigmoid(mutxks))*self.update_weights
        self.S_t = self.S_t + \
            self.update_xks @ (self.S_t_factors * self.update_xks).T
        self.mu_sum = np.zeros_like(self.mean)
        self.mu_sum += np.sum(self.update_weights * (self.update_Ys - sigmoid(mutxks))
                              * self.update_xks, axis=1, keepdims=True)
        inv_cov_0 = scipy.linalg.inv(self.cov_0)
        new_cov = scipy.linalg.inv(inv_cov_0 + self.S_t)
        new_mean = new_cov @ (inv_cov_0 @ self.mean_0 +
                              self.S_t @ self.mean + self.mu_sum)
        # Added prior to the mean
    ...
    def compute_mean_scipy(self, xks, Ys):
        ...
        def loss_function(mean):
            mutxks = mean.reshape(-1, 1).T @ xks
            probabilities = sigmoid(mutxks)
            # print(probabilities)
            return -np.sum((Ys*np.log(probabilities) + (1-Ys)*np.log(1-probabilities)))
    ...
    # def get_mean_rewards(self, size=1000):
    ...
    #     return np.mean(reward_samples, axis=0)
